...
Базовая схема:
    C source
    ↓
    Clang Frontend / Препроц
    ↓
    LLVM IR  / Платформонезависимо
    ↓
    LLVM x86-64 Backend
    ↓
    ELF relocatable object (.o) / ASM
*** Тесты препроцессора
test.c
#+begin_src c
#define HELLO "World"
...
#+begin_src  shell :results output code :dir ../CEd/Practice
llvm-clang -target x86_64-unknown-linux-gpu -S test.c -o test.s
cat test.s
#+end_src
...
*** Note
Дальше этого на mac я не уйду без libc / CRT
libc для библиотек
CRT - c runtime который получает управление после ядра, берет аргументы, инициализирует libc
